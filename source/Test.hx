...
class Test extends Action
{
	...
	override public function create()
	{
		...
		i = new UIInputTfCore(200, "Contractor", bottom);
		j = new UIInputTfCore(200, "Voip", bottom);
		super.create();
		//this.question.text = "test";
		i.addToParent(this);
		j.addToParent(this);
		i.positionMe(new Rectangle(this.question.x, this.question.y, this.question.width, this.question.height));
		j.positionMe(new Rectangle(i.x, i.y, i.width, i.height));
		i.setStyle();
		j.setStyle();
	}
	...
	function onPaste(e):Void
	{
		...
		//trace(content);
		var contract = parseCustomerProfile(content);
		//trace(contract == null);
		if (contract != null)
		{
			trace(contract);
			...
			{
				i.inputtextfield.text = contract.contractorID;
			}
			if (contract.voip != "")
			{
				j.inputtextfield.text = contract.voip;
			}
		}
		var techSetup = parseHealthCheckDashboard(content);

	}
	function parseCustomerProfile(content:String):Contractor
	{
		//trace(content);
		var profile:Map<String,Map<String,String>> = [];
		var t:Array<String> = content.split("\n");
		//trace(t);
		var getLangEreg:EReg = new EReg("^PROD(EN|FR|IT|DE)","");
		var LANG = "";
		...
		var addressEreg:EReg = new EReg("^[0-9]+\\s[\\S\\s-.]+,\\s[0-9]{4}\\s[\\S\\s-.]+$","i");
		...
		var topics = [
			"FR" => [
				...
				"owner"=>[
					...
					"Validation de l'adresse email"=>{universal:"vtiOwnerEmailValidated",ereg:~/^[\s\S]+/},
				],
				"payer" => [
			...
			"IT" => [
				...
				"owner"=>[
					"Owner" => {universal:"vtiOwner", ereg: personEreg},
					"Email" => {universal:"vtiOwnerEmail", ereg:emailReg},
					"Email validation"=>{universal:"vtiOwnerEmailValidated",ereg:~/^[\s\S]+/},
				],
				"payer" => [
			...
			"DE" => [
				...
				"owner"=>[
					"Owner" => {universal:"vtiOwner", ereg: personEreg},
					"Email" => {universal:"vtiOwnerEmail", ereg:emailReg},
					"Validierung der E-Mail-Adresse"=>{universal:"vtiOwnerEmailValidated",ereg:~/^[\s\S]+/},
				],
				"payer" => [
	...
	public function parseHealthCheckDashboard(s:String)
	{
		...
		var ontSubset:Map<String,EReg> = [
			...
			"ARC_WAN_1_IP6_StaticGateway"=>new EReg("^[\\s\\S]+$","")
			];
		...
		for (j in n)
		{
			...
			else
			{
				...
				var tmpReg:EReg=new EReg("^[\\s\\S]+$","");
				//trace(currentSet);
				//trace(currentSubSet);
				//trace(j);
				if (getLangEreg.match(j)){
					//trace(getLangEreg.matched(1));
					dataSet.get("META").set("lang", getLangEreg.matched(1));
				}
